...
export function AttendanceReportTable({ data }: AttendanceReportTableProps) {
    if (data.length === 0) {
        return <div className="text-muted-foreground text-center py-8 text-xs font-bold uppercase tracking-widest">No matching telemetry data</div>
    }
        ...
        <div className="space-y-4">
            ...
            <div className="hidden laptop:block bg-background/40 border border-border rounded-2xl overflow-hidden backdrop-blur-sm shadow-2xl">
                <div className="px-4 py-3 border-b border-border bg-white/[0.02] flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Attendance Telemetry</span>
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="border-border hover:bg-transparent">
                            ...
                            <TableHead className="text-[9px] uppercase font-black tracking-widest text-muted-foreground text-center h-10">Presence</TableHead>
                            ...
                            <TableRow key={row.student.id} className="border-border hover:bg-white/[0.02] transition-colors group">
                                <TableCell className="py-3 text-[11px] font-bold text-foreground group-hover:text-foreground transition-colors">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-muted-foreground uppercase font-black">ID: {row.student.id.slice(0, 8)}</span>
                                    </div>
                                </TableCell>
                                ...
                                <TableCell className="py-3 text-center text-muted-foreground text-[10px] font-bold">{row.absentCount}</TableCell>
                                <TableCell className="py-3 text-center text-muted-foreground text-[10px] font-bold">{row.lateCount}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
            ...
            <div className="grid grid-cols-1 tablet:grid-cols-2 gap-4 laptop:hidden">
                {data.map((row) => (
                    ...
                    >
                        ...
                        <div className="w-full h-1.5 bg-background rounded-full overflow-hidden mb-5">
                            <div
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="bg-background/50 p-2 rounded-lg border border-border text-center">
                                <p className="text-[9px] text-muted-foreground font-black uppercase tracking-tighter">Absent</p>
                                <p className="text-sm font-black text-red-400">{row.absentCount}</p>
                            </div>
                            <div className="bg-background/50 p-2 rounded-lg border border-border text-center">
                                <p className="text-[9px] text-muted-foreground font-black uppercase tracking-tighter">Late</p>
                                <p className="text-sm font-black text-amber-400">{row.lateCount}</p>
                            </div>
                            <div className="bg-background/50 p-2 rounded-lg border border-border text-center">
                                <p className="text-[9px] text-muted-foreground font-black uppercase tracking-tighter">Excused</p>
                                <p className="text-sm font-black text-primary">{row.excusedCount}</p>
                            </div>
                        </div>
                    </div>
                ))}
